...
from lsp_select.planners import LSPCycleGAN, RuntimeMonitoringPlanner
...
def maze_eval(args):
    ...
    with builder(args.unity_path) as unity_bridge:
        ...
        planner = RuntimeMonitoringPlanner(goal, planners, args.priors, args.threshold, args)
        ...
        for _, step_data in enumerate(planning_loop):
            ...
            if args.do_plot:
                ...
                plt.title(f'RTM - {planner.chosen_planner_name} ({args.env})')
                plt.xlabel(f'seed={args.current_seed}, threshold={args.threshold}\n'
                           f'Cross Entropy: LSP={planner.xentropy_avg[0]:.6f}, '
                           f'LSPCycleGAN={planner.xentropy_avg[1]:.6f}')
                plt.show()
                plt.pause(0.01)
...
if __name__ == "__main__":
    ...
    args.planner = 'rtm'
    prior_no_of_obs = 5000
    args.priors = np.array([[2.0 * prior_no_of_obs, prior_no_of_obs],
                           [2.0 * prior_no_of_obs, prior_no_of_obs]])
    ...
    for seed in range(*args.seed):
        ...
        with open(priors_file, 'w') as f:
            np.savetxt(f, args.priors.reshape(1, -1))
