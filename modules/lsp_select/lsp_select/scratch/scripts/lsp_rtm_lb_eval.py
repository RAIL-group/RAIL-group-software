...
from lsp_select.planners import RTMLowerBound
...
def maze_eval(args):
    ...
    with builder(args.unity_path) as unity_bridge:
        ...
        planner = RTMLowerBound(goal, planners, args.priors, args)
        ...
        for _, step_data in enumerate(planning_loop):
            ...
            if args.do_plot:
                ...
                plt.title(f'RTMLB - {planner.chosen_planner_name}[{planner.chosen_planner_idx}] ({args.env})')
                plt.xlabel(f'seed={args.current_seed}\n'
                           f'Avg. Cost: Dijkstra={planner.avg_estimated_costs[0]:.2f}, '
                           f'LSP-A={planner.avg_estimated_costs[1]:.2f}, '
                           f'LSP-B={planner.avg_estimated_costs[2]:.2f}, '
                           f'LSP-C={planner.avg_estimated_costs[3]:.2f}')
                plt.show()
                plt.pause(0.01)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--network_files', type=str, nargs='+')
    ...
    for seed in range(*args.seed):
        ...
        with open(Path(args.save_dir) / f'selected_{args.planner}_{args.env}_{seed}.txt', 'w') as f:
            f.write(f'{planner.chosen_planner_name}[{planner.chosen_planner_idx}]')
        args.priors = priors
