...
def test_lsp_cyclegan(unity_path, do_debug_plot, lsp_select_network_file, lsp_select_generator_network_file):
    ...
    args.generator_network_file = lsp_select_generator_network_file

    if args.network_file is None:
        pytest.xfail("Argument lsp_select_network_file is required.")
    if args.generator_network_file is None:
        pytest.xfail("Argument lsp_select_generator_network_file is required.")
    ...
    with builder(args.unity_path) as unity_bridge:
        ...
        planner = LSPCycleGAN(goal=goal, args=args)
        ...
        for _, step_data in enumerate(planning_loop):
            ...
            if do_debug_plot:
                ...
                plt.title('Real View')
                plt.subplot(312)
                plt.imshow(planner.observation['image'])
                plt.title('CycleGAN Transformed View')
                ax = plt.subplot(313)
