...
# AIM for this: For 2 robots (to be specific), do the same. Make sure the two robots (red, and blue)
# , they follow the same path and use learning.
...
def navigate_unknown(args, termination=any, num_robots=2, do_plot=True):
    ...
    with builder(args.unity_path) as unity_bridge:
        ...
        planner = mrlsp.planners.MRLearnedSubgoalPlanner(robots, goal_pose, args)
        ...
        # path,image,subgoal,visibility mask to store the path taken by every robot:
        # This is only used for plotting and updating the planner
        ...
        while (not termination(goals_reached)):
            for i, iter_planning_loop in enumerate(iter_planning_loops):
                ...
                try:
                    ...
                    planning_loops[i].update_robot_grid(robot_grid)

                # if next(iter_planning_loop) raises exception, (means goal is reached for that robot) just pass it
                #  and plot using previous data of robot
    ...
    cost_file = Path(args.save_dir) / f'planner_mrlsp_{args.map_type}_cost_{args.current_seed}_r{num_robots}.txt'
